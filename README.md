...
заранее поднятый стек им не нужен, достаточно запущенного Docker. Dev-ключи RS256, на которых
auth-service подписывает токены, выпускает задача `generateDevJwtKeys` — отдельного шага
